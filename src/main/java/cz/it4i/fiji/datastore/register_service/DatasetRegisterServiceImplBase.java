...
public abstract class DatasetRegisterServiceImplBase<T> {
    ...
    protected List<cz.it4i.fiji.datastore.register_service.ResolutionLevel>
    ...
    protected cz.it4i.fiji.datastore.register_service.ResolutionLevel
    ...
    protected CreateNewDatasetTS.N5Description convert(DatasetDTO dataset) {
// @formatter:off
        CreateNewDatasetTS.N5Description.N5DescriptionBuilder result = CreateNewDatasetTS.N5Description.builder()
...
//	@formatter:on
    ...
    protected AffineTransform3D[] createTransfoms(int angles,
    ...
    protected @NonNull Compression createCompression(String compression) {
        return getCompressionMapping().getOrDefault(compression.toUpperCase(),
                new RawCompression());
    }

    protected Dataset getDataset(String uuid) {
        Dataset dataset = datasetDAO.findByUUID(uuid);
        return dataset;
    }

    protected Map<String, Compression> getCompressionMapping() {
    ...
    protected void illegalVersionAndModeCombination(String version,
                                                  OperationMode mode)
    {
    ...
    protected int resolveVersion(Dataset dataset, String version,
	...
	/*protected String toString(int[] resolution) {
		return "[" + IntStream.of(resolution).mapToObj(i -> Integer.toString(i))
			.collect(Collectors.joining(",")) +
			"]";
	}*/

    protected static ExportMipmapInfo createExportMipmapInfo(
    ...
    protected static ExportScalePyramid.LoopbackHeuristic createLoopbackHeuristic(boolean isVirtual) {

        final long planeSizeInBytes = -1;
        final long maxMemory = Runtime.getRuntime().maxMemory();
        return new ExportScalePyramid.LoopbackHeuristic() {
    ...
    protected static ExportScalePyramid.AfterEachPlane createAfterEachPlane(final boolean isVirtual,
                                                                          Runnable clearCache)
    {
        return new ExportScalePyramid.AfterEachPlane() {
    ...
    protected void mergeVersions(Dataset dataset) throws IOException {
